  ...
  if (isHit && bases <= 2 && isOutfieldHit) {
    ...
    if (bases === 2 && preBases[0]) {
      ...
      if (runnerAt3rd && runnerAt3rd.name === preBases[0].name) {
        ...
        const hc = (0.30 + sf * 0.58 - armPenalty - positioningPenalty) * outsMultiplier;
    ...
    if (bases === 1 && preBases[1]) {
      ...
      if (runnerAt3rd && runnerAt3rd.name === preBases[1].name) {
        ...
        const hc = (0.38 + sf * 0.64 - armPenalty * 0.9 - positioningPenalty * 0.8 + twoOutBonus) * outsMultiplier;
...
export function resolvePitch(state, pitchType) {
  ...
  // Offensive tuning: lower base strike rate so more balls are thrown, pushing
  // walk rate toward 2.5-2.8/team/game. Wild pitchers (low control) punished more.
  let strikeChance = 0.25 + controlFactor * 0.27 + (pitchType.controlBonus || 0) * 0.04;
...
export function resolveSwing(state, swingType, pitch) {
  ...
  if (Math.random() < hitChance) {
    ...
    } else if (adjBatter.speed >= 4 && hr2 < (effPwr * 0.15 + sf2 * 0.04) * doubleMod) {
    ...
    if (isGrounder) {
      ...
      if (state.bases[2] && state.outs < 2) {
        ...
        const prodC = isRS2 ? 0.55 + (r3p.speed / 10) * 0.28 : 0.25 + (r3p.speed / 10) * 0.18;
