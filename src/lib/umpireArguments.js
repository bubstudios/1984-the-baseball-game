...
export function getArgumentSeverity(lastPlay, gameState, usedTopics) {
  ...
  if (["flyout","popout","sacfly"].includes(type) && r < 0.04) {
    if (!capped("trappedBall", 1))
      return arg(pick(["trapped or clean catch?","outfielder claims he caught it","ball glanced off the wall first","did he squeeze it before it hit the wall?","catch made in the crowd — fan interference?"]), "low", 2, "trappedBall", "major");
  }
