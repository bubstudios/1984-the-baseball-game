...
export default function IndiansBannerPopup({ entry, onDismiss, onAchievement }) {
  const [visible, setVisible] = useState(false);

  useEffect(() => {
    setVisible(true);
    const unlocked = trackIndiansBannerView(entry.id);
  ...
  const categoryColors = {
    bobblehead: 'from-red-700 to-orange-600',
    poster: 'from-red-800 to-red-700',
    tradition: 'from-slate-600 to-red-700',
    historic: 'from-orange-700 to-red-700',
    inspiration: 'from-red-600 to-orange-600',
    community: 'from-red-700 to-amber-600',
    ballpark: 'from-slate-700 to-red-600',
    fans: 'from-red-600 to-orange-500',
  };
    ...
    <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center" onClick={onDismiss}>
      ...
      >
        ...
        <div className="bg-black/40 px-5 py-2.5 flex justify-between items-center border-t border-white/20">
          <span className="text-[9px] text-white/50 font-heading uppercase tracking-wider">🇺🇸 Indians Tradition</span>
