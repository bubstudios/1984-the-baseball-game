...
import { trackNationalWrestlingView } from '@/lib/nationalWrestlingPopups';

export default function NationalWrestlingPopup({ entry, onDismiss, onAchievement }) {
  ...
  useEffect(() => {
    if (visible && entry?.id) {
      const unlocked = trackNationalWrestlingView(entry.id);
    ...
    <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center" onClick={onDismiss}>
      ...
      >
        ...
        <div className="bg-[#0d0d1a] px-5 py-2.5 flex justify-between items-center border-t" style={{ borderColor: entry.color + '10' }}>
          <span className="text-[9px] text-muted-foreground/40 font-heading uppercase tracking-wider">Wrestling • 1984</span>
