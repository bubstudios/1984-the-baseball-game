...
export default function BlueJaysBannerPopup({ entry, onDismiss, onAchievement }) {
  const [visible, setVisible] = useState(false);

  useEffect(() => {
    setVisible(true);
    const unlocked = trackBluejaysBannerView(entry.id);
  ...
  const categoryColors = {
    bobblehead: 'from-blue-600 to-cyan-600',
    poster: 'from-blue-700 to-indigo-600',
    tradition: 'from-slate-600 to-blue-700',
    historic: 'from-indigo-600 to-purple-600',
    inspiration: 'from-cyan-600 to-blue-600',
    community: 'from-blue-600 to-teal-600',
    ballpark: 'from-sky-600 to-blue-600',
    fans: 'from-blue-500 to-cyan-600',
  };
    ...
    <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center" onClick={onDismiss}>
      ...
      >
        ...
        <div className="bg-black/40 px-5 py-2.5 flex justify-between items-center border-t border-white/20">
          <span className="text-[9px] text-white/50 font-heading uppercase tracking-wider">🇨🇦 Blue Jays Pride</span>
